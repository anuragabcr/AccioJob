...
function onSearch() {
    ...
    let temp = students.filter((student) => {
        return student.first_name.toLowerCase().includes(search.toLowerCase()) || student.last_name.toLowerCase().includes(search.toLowerCase()) || student.email.toLowerCase().includes(search.toLowerCase())
    })
    createTable(temp)
}
...
function sortData(filter) {
    switch (filter) {
        ...
        case 'gender':
            if (gender) {
                students.sort(function (a, b) {
                    let nameA = a.gender.toLowerCase()
                    let nameB = b.gender.toLowerCase()
                    if (nameA < nameB) {
                        return -1
                    } 
                    if (nameA > nameB) {
...
window.onload = createTable(students)